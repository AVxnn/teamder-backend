...
router.post("/", isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    const { minRating, maxRating, preferredHeroes, preferredRoles } = req.body;

    if (!telegramId) {
      return res.status(400).json({ error: "telegramId is required" });
    }

    // Преобразуем параметры запроса в нужные типы
    ...
    if (preferredHeroes) {
      filters.preferredHeroes = Array.isArray(preferredHeroes)
        ? preferredHeroes.map(String)
        : [String(preferredHeroes)];
    }
    if (preferredRoles) {
      filters.preferredRoles = Array.isArray(preferredRoles)
        ? preferredRoles.map(String)
        : [String(preferredRoles)];
    }

    console.log("Applied filters:", filters); // Для отладки
...
router.post("/like", isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    const result = await recommendationService.likeUser(
      req.body.telegramId,
      toTelegramId
    );
    res.json(result);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await recommendationService.superLikeUser(
        req.body.telegramId,
        toTelegramId
      );
      res.json(result);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await recommendationService.dislikeUser(
        req.body.telegramId,
        toTelegramId
      );
      res.json(result);
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      const likesInfo = await recommendationService.getLikesInfo(
        req.body.telegramId
      );
      res.json(likesInfo);
    } catch (error: any) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!amount || !starsCost) {
        return res
          .status(400)
          .json({ error: "amount and starsCost are required" });
      }

      const result = await recommendationService.purchaseExtraLikes(
        req.body.telegramId,
        amount,
        starsCost
      );
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const result = await recommendationService.purchaseExtraSuperLikes(
        req.body.telegramId,
        amount,
        starsCost
      );
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const likes = await recommendationService.getLikesReceived(
        req.body.telegramId
      );
      res.json(likes);
    } catch (error: any) {
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const likes = await recommendationService.getLikesGiven(
        req.body.telegramId
      );
      res.json(likes);
    } catch (error: any) {
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const likesInfo = await recommendationService.getLikesBetweenUsers(
        req.body.telegramId,
        toTelegramId
      );
      res.json(likesInfo);
